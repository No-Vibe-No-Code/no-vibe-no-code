...
document.getElementById("langToggle").onclick = () => { lang = lang === "en" ? "zh" : "en"; setLanguage(); };
setLanguage();
const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
const animeEngine = window.anime;
const field = document.getElementById("particleField");
if (field && animeEngine && !reduceMotion) {
  const context = field.getContext("2d");
  const points = Array.from({ length: 90 }, (_, index) => ({ x: Math.random(), y: Math.random(), size: index % 4 === 0 ? 5 : 3, alpha: 0.14 + Math.random() * 0.25, drift: 8 + Math.random() * 20 }));
  const resize = () => { field.width = window.innerWidth * devicePixelRatio; field.height = window.innerHeight * devicePixelRatio; context.setTransform(devicePixelRatio, 0, 0, devicePixelRatio, 0, 0); };
  resize(); window.addEventListener("resize", resize, { passive: true });
  const draw = () => { context.clearRect(0, 0, window.innerWidth, window.innerHeight); points.forEach((point) => { const x = point.x * window.innerWidth; const y = point.y * window.innerHeight; context.fillStyle = `rgba(232,62,131,${point.alpha})`; context.beginPath(); context.arc(x, y, point.size, 0, Math.PI * 2); context.fill(); }); points.forEach((point, index) => { const x = point.x * window.innerWidth; const y = point.y * window.innerHeight; points.slice(index + 1).forEach((other) => { const ox = other.x * window.innerWidth; const oy = other.y * window.innerHeight; const distance = Math.hypot(x - ox, y - oy); if (distance < 145) { context.strokeStyle = `rgba(232,62,131,${0.08 * (1 - distance / 145)})`; context.beginPath(); context.moveTo(x, y); context.lineTo(ox, oy); context.stroke(); } }); }); };
  draw();
  animeEngine.animate(points, { y: () => Math.random(), duration: (point) => point.drift * 1000, ease: "inOutSine", loop: true, alternate: true, onUpdate: draw });
}
if (animeEngine && !reduceMotion) {
  animeEngine.animate(".hero-copy > *", { translateY: [16, 0], opacity: [0, 1], delay: animeEngine.stagger(90), duration: 800, ease: "out(4)" });
  animeEngine.animate(".overview-block, .feature-grid article, .rhythm-card, .competition-body > *, .panel-form", { translateY: [20, 0], opacity: [0, 1], delay: animeEngine.stagger(70), duration: 700, ease: "out(4)", autoplay: true });
}
