...
Object.assign(copy.zh, { heroQrTitleText: "用微信扫码。<br><span>从这里开始。</span>", heroQrText: "直接加入创始群，查看 Club Fair 计划，并在第一次活动确认后收到时间和教室信息。", heroQrMeta: "当前邀请 · 有效至 9 月 20 日" });
Object.assign(copy.en, { createAccount: "Create account", mobileJoin: "Create account", firstMeetNote: "Create an account now and we will share the Club Fair plan, first-meeting time, and room as soon as they are confirmed.", joinNowText: "1. Create your account<br>2. Join the group<br>3. Bring one idea" });
Object.assign(copy.zh, { createAccount: "创建账号", mobileJoin: "创建账号", makeKicker: "任何东西", makeTitle: "想到什么。<br><span>就做出来。</span>", firstMeetNote: "现在创建账号，Club Fair 计划、第一次活动时间和教室确认后会第一时间通知。", joinNowText: "1. 创建账号<br>2. 加入群聊<br>3. 带一个想法" });
...
const joinAccountButton = document.getElementById("joinAccountButton");
...
const returnTo = requestedReturnTo.startsWith("/") && !requestedReturnTo.startsWith("//") ? requestedReturnTo : "/home.html";
...
const restoreAccount = () => requestJson("/api/me").then(({ user }) => {
  ...
  location.replace(returnTo === "/home.html" ? "/home.html" : returnTo);
