...
const inputFor = (field) => {
  ...
  if (["single-choice","checkboxes"].includes(field.type)) return `<div class="stack">${(field.options || []).map((option,index) => `<label><input type="${field.type === "checkboxes" ? "checkbox" : "radio"}" id="${escapeHtml(field.id)}${index ? '-'+index : ''}" name="${escapeHtml(field.id)}"${field.required && field.type === 'single-choice' ? ' required' : ''} value="${escapeHtml(option)}"> ${escapeHtml(option)}</label>`).join("")}</div>`;
...
document.getElementById("dynamicForm").onsubmit=async(event)=>{event.preventDefault();const data=new FormData(event.target),answers={};for(const field of (form.schema.sections||[]).flatMap(section=>section.fields||[])){if(field.type==='checkboxes')answers[field.id]=data.getAll(field.id);else if(field.type==='consent')answers[field.id]=data.has(field.id);else if(data.has(field.id))answers[field.id]=data.get(field.id);}try{await requestJson(`/api/forms/${encodeURIComponent(form.id)}/responses`,jsonOptions("POST",{answers}));event.target.innerHTML='<div class="empty-state"><h3>Response submitted</h3><p>Your response has been saved.</p><a class="primary-button" href="/home.html">Member home</a></div>';Motion.revealWithin(event.target);}catch(error){document.getElementById("responseMessage").textContent=error.message;}};
load().catch((error)=>{
