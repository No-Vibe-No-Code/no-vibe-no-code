...
memberCount.className = 'collection-result-count';
memberCount.setAttribute('role', 'status');
document.getElementById('members').before(memberCount);
const render = () => {
  ...
  memberCount.textContent = `${members.length} ${members.length === 1 ? 'member' : 'members'}${query ? ` matching “${query}”` : ''}`;
...
load().catch((error) => { memberCount.textContent = 'Members could not be loaded.'; document.getElementById("members").innerHTML = `<div class="notice error">${escapeHtml(error.message)}</div>`; });
