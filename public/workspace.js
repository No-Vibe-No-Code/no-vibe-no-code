(() => {
  ...
  let user,preferences={},current,items=[],cursor=null,total=0,generation=0,dirty=false,initialized=false,searchTimer,inviteTimer,lastOpened,preferenceQueue=Promise.resolve();
  ...
  async function renderProject(version) {
    ...
    if(tab==='editor'&&project.canEdit){root.insertAdjacentHTML('beforeend',`<div class="ws-editor">${upload('cover',project.coverUrl)}<form class="ws-form" data-form="project" style="margin-top:24px">${projectFields(project)}${formEnd('Save changes')}</form></div>`);return;}
  ...
  async function renderNotifications(version) {
    ...
    root.innerHTML=heading('Notifications','Your invitations, project updates, and club news.',button('read-all','Mark all read'))+tabs(['inbox','unread','saved','done'].map(name=>[name,name[0].toUpperCase()+name.slice(1),data.counts[name]]),filter,'filter')+`<div class="ws-toolbar">${search('Search notifications',q().get('q'))}${select('type','Notification category',[['','All categories'],['team-invitation','Team invitations'],['project-status','Project updates'],['broadcast','Club announcements']],q().get('type')||'')}</div><div class="ws-inbox-layout ${current?'has-selection':''}"><div class="ws-inbox-main"><div class="ws-bulk"><label><input type="checkbox" aria-label="Select all notifications on this page" data-select-all ${items.length&&items.every(i=>selected.has(i.id))?'checked':''}> <span id="selectionCount">${selected.size?selected.size+' selected':'Select page'}</span></label>${['read','unread','save','unsave','done','restore'].map(action=>button('bulk',({read:'Read',unread:'Unread',save:'Save',unsave:'Unsave',done:'Done',restore:'Restore'})[action],`data-value="${action}" ${selected.size?'':'disabled'}`)).join('')}</div><div class="ws-result-bar">${total} notifications</div><section class="ws-notice-list" id="noticeItems">${items.length?items.map(noticeRow).join(''):empty('All caught up','Nothing in this view. Check another filter or come back later.')}</section>${loadMore()}</div><aside class="ws-notice-detail" aria-label="Notification details">${noticeDetail(current)}</aside></div>`;
  }
  ...
  root.addEventListener('click',async event=>{
    ...
    try {
      ...
      if(action==='open-notice'){await request(`/api/notifications/${encodeURIComponent(id)}/read`,json('POST',{}));lastOpened=id;navigate({notification:id});refreshUnread();}
      if(action==='close-notice')navigate({notification:null});
